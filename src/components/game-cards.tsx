    ...
    <div className="space-y-4">
      {games.map((game) => (
        <Link
          key={game.id}
          href={`/games/${game.id}`}
          className="block border border-gray-700 bg-gray-900 p-5 transition hover:bg-gray-800"
        >
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1">
              ...
              <h3 className="mt-2 text-lg font-semibold text-white">{buildGameTitle(game)}</h3>
          ...
          <p className="mt-4 line-clamp-2 text-sm leading-5 text-gray-300 font-mono">
            {game.pgn}
          </p>
        </Link>
      ))}
